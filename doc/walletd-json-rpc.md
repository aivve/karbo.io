...
More on how to start and operate Karbowanec RPC Wallet can be found here: [Karbowanec RPC Wallet](karbo_walletd.md)
...
**getStatus()** method returns information about the current RPC Wallet state: block\_count, known\_block\_count, last\_block\_hash, peer\_count an minimal\_fee.
...
} ``` Output value example: ``` { "id": "test", "jsonrpc": "2.0", "result": {} } ```
...
| items    | Array that contains:                                                                      
            ...
            -   transactionHashes - array - array of strings, where each string is a transaction hash  | array  | See below |
