...
for (var i = 0; i < crypt_multi_num_cur; ++i) {
    ...
    if (getPar.success != false) {
        ...
        var month_name = new Array("січня", "лютого", "березня", "квітня", "травня", "червня", "липня", "серпня", "вересня", "жовтня", "листопада", "грудня");
        ...
        document.write('<div style="padding:10px;">Курси недоступні</div>');
    }
}

if (crypt_multi_display_time)
    document.write('<div style="font-size:70%;opacity:0.6;padding-top:10px;padding-left:10px;">Оновлено ' + day + ' ' + month + ' ' + year + ' р. '
        + hour + ':' + minutes + ':' + seconds + ' '
        + time_zone + '</div>');

document.write('<div style="font-size:10px;opacity:0.6;padding:10px;">Дані з <a style="color:' + crypt_multi_font_color + ';" href="https://www.cryptonator.com" target="_blank">Cryptonator</a></div>\
</div>');