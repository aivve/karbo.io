...
$.when(
	...
	$('#price_pln').html('<strong>' + (Math.round(price_pln * 100)/100).toFixed(2) + '</strong>' + ' zł');
	...
	calculateRate = function() {	
        ...
        var uah = Math.round(krb * price_uah * 100)/100;
	    $('#inputAmountUAH').val(uah);
	};
	...
	calculateReverseRate = function() {	
        var uah = parseFloatString(document.getElementById('inputAmountUAH').value);
        var krb = Math.round(uah / price_uah * 100)/100;
	    $('#inputAmountKRB').val(krb);
	};
	
	$('#inputAmountUAH').on('input',function(e) { 
